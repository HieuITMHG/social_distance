...
class YouTubeCameraWorker(ImprovedCameraWorker):
    def _open_video_source(self):
        """Open or reopen video source, chỉ dành cho YouTube livestream"""
        try:
            source = self.config.source
            if isinstance(source, str) and source.startswith('http') and 'youtube.com' in source:
                import yt_dlp
                ydl_opts = {
                ...
                # fallback: gọi lại logic gốc của ImprovedCameraWorker
                super()._open_video_source()
            ...
            if self.cap is not None and self.cap.isOpened():
                ...
                if hasattr(self, 'is_video_file') and self.is_video_file:
                    self.total_frames = int(self.cap.get(cv2.CAP_PROP_FRAME_COUNT))
                    self.logger.info(f"Opened video file: {self.config.source}, Total frames: {self.total_frames}, FPS: {fps}")
                self.tracker.current_fps = max(fps, 1)
                self.target_fps = min(fps, 30)
                self.frame_time = 1.0 / self.target_fps
                if self.video_writer is None and self.config.enable_recording:
                    from datetime import datetime
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    output_path = os.path.join(self.config.recording_path, f"{self.config.camera_id}_{timestamp}.mp4")
                    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                    self.video_writer = cv2.VideoWriter(output_path, fourcc, fps, (width, height))
            else:
                self.logger.error(f"Cannot open camera source: {self.config.source}")
...
class MultiCameraSurveillanceSystem:
    """Main surveillance system with improved real-time display (không vẽ UI)"""
    ...
    def start(self):
        """Start the surveillance system (không vẽ UI)"""
        ...
        for camera_id, config in self.cameras.items():
            if config.source.startswith('http') and 'youtube.com' in config.source:
                worker = YouTubeCameraWorker(config, self.batch_processor, self.db_manager)
            else:
        ...
        threads = [
            ...
            # threading.Thread(target=self._display_loop, daemon=True)  # Bỏ display loop
        ]
